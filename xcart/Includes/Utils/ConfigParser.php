...
abstract class ConfigParser extends \Includes\Utils\AUtils
{
    ...
    protected static function adjustHost()
    {
        foreach (['http_host', 'https_host'] as $host) {
            ...
            $hostValue = static::$options['host_details'][$host] ?? null;
            $adminHost = static::$options['host_details']['admin_host'] ?? null;

            if (
                !empty($adminHost)
                && $adminHost !== $hostValue
                && defined('XCN_ADMIN_SCRIPT')
            ) {
                static::$options['host_details'][$host . '_orig'] = $hostValue;
                static::$options['host_details'][$host] = $adminHost;
            } elseif (
                !empty(static::$options['host_details']['domains'])
                && $hostValue !== $_SERVER['HTTP_HOST']
