...
abstract class ErrorHandler
{
    ...
    public static function logException($exception)
    {
        try {
            ...
            static::logInfo($e->getMessage(), $e->getCode(), self::getExceptionTraceAsString($e));
        }

        static::logInfo($exception->getMessage(), $exception->getCode(), self::getExceptionTraceAsString($exception));
    }

    /**
     * Custom backtrace to sting method
     *
     * @param      $exception
     * @param bool $useArgs
     *
     * @return string
     */
    protected static function getExceptionTraceAsString($exception, $useArgs = false)
    {
        $string   = "";
        $count = 0;
        foreach ($exception->getTrace() as $frame) {
            $args = "";
            if ($useArgs && isset($frame['args'])) {
                $args = [];
