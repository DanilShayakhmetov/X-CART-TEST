...
class I18N_Arabic_Glyphs
{
    ...
    protected function preConvert($str)
    {
        ...
        for ($i = $max - 1; $i >= 0; $i--) {
            ...
            $isAsciiNextChar = isset($chars[$i - 1]) ? ord($chars[$i - 1]) < 128 : true;
            $isAsciiPrevChar = isset($chars[$i + 1]) ? ord($chars[$i + 1]) < 128 : true;
            ...
            if ($crntChar && mb_strpos($flip_arr, $crntChar) !== false && !$isAsciiNextChar) {
            ...
            if ($crntChar && !$Reversed 
                && (mb_strpos($ReversedChr, $crntChar) !== false && !$isAsciiPrevChar)
            ) {
